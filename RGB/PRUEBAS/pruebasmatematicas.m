...
restriccionb=255;
direcction='E:\CONOCIMIENTO\UNIVERSIDAD\INSTRUMENTACION\PROYECTO I (ESTACION DE MEDIDA BIOMETRICA)\MATLAB\INTERFAZ\TRATAMIENTO DE IMAGEN';
...
for row=1:sz(1)
    for col=1:sz(2)
        ...
        a(row,col,2)=imp(row,col,1)/imp(row,col,3);
        a(row,col,3)=imp(row,col,2)/imp(row,col,3);
    end
end
xtoy=sz(1)*sz(2);
% 
...
imskin=((im(:,:,1)./im(:,:,2))>rangosdecolor(1,2) &...
    ...
    (im(:,:,1)./im(:,:,3))<rangosdecolor(2,1) & im(:,:,3)<restriccionb  ).*...
    ((im(:,:,2)./im(:,:,3))>rangosdecolor(3,2) &...
    (im(:,:,1)./im(:,:,3))<rangosdecolor(3,1) & im(:,:,3)<restriccionb );
